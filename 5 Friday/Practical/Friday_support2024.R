...
crop_param <- quefts_crop_parameters(
  crop_id = crop_id,
  crop_pars = crop_pars
)[[1]]
...
## Step 1: quantile
quantile(
  x = 100:300, # A vector with values
  probs = c(0.05, 0.95), # The under- and upper quantile
  names = FALSE
) # Force to return plain numbers
...
ra_below_threshold <- app(rs_yield_realisations,
  fun = function(x) {
    100 * mean(x < threshold, na.rm = TRUE)
  }
)
...
df_mean_per_region_per_realisation <- cbind(
  division_names,
...
df_yield_sd_per_region <-
  data.frame(
    ...
    yield_sd =
      apply(
        X = df_mean_per_region_per_realisation[, -(1:2)], ## apply over all columns except the first two
...
n_yield_pred_int_width_whole_area <- diff(
  quantile(
    x = vn_average_yield_per_realisation,
    probs = c(0.05, 0.95),
    names = FALSE
  )
)
...
rs_plot <- c(
  ra_yield_pred_interv,
  ra_yield_pred_interv_per_division,
  ra_yield_pred_interv_whole
)
names(rs_plot) <- c(
  "Pixel support 90% PIW",
...
plot(ra_yield_pred_interv_per_division,
  main = "Division support 90% PIW",
  range = c(75, 1225), type = "continuous"
)
lines(sppol_divisions)
plot(ra_yield_pred_interv_whole,
  main = "Global support 90% PIW",
  range = c(75, 1225), type = "continuous"
)
par(mfrow = c(1, 1))
