...
#### get and plot data ####
setwd("4 Thursday/Practical/") # path to data folder
...
x11()
plot(OCSstack[[1]], range = c(18, 155), main = 'Original map') # holes are rock outcrops, water & urban
...
set.seed(12345)
idx <- as.integer(spatSample(OCSstack[[1]], 2500,
  method = "random",
  na.rm = T, cells = T
)[, 1])
...
RFpredict <- predict(OCSstack, RFmodel,
  fun = predfun,
...
x11()
plot(RFpredict, range = c(18, 155), main = 'Predicted')
...
RMSE <- global(resids, function(x) {
  sqrt(mean(x^2, na.rm = TRUE))
})[[1]] # RMSE
SSR <- global(resids, function(x) {
  sum(x^2, na.rm = TRUE)
})[[1]] # squared res
mref <- global(OCSstack[[1]], "mean", na.rm = TRUE)[[1]] # mean reference OCS
SST <- global(OCSstack[[1]], function(x) {
  sum((x - mref)^2, na.rm = TRUE)
})[[1]]
MEC <- 1 - SSR / SST
...
set.seed(6789)
idx <- as.integer(spatSample(OCSstack[[1]], 350,
  method = "random",
  na.rm = T, cells = T
)[, 1])
...
set.seed(12345)
idx <- as.integer(spatSample(OCSstack[[9]], 50,
  method = "stratified",
  na.rm = T, cells = T
)[, 1])
...
set.seed(12345)
for (strat in 1:7) {
  idx <- as.integer(spatSample(OCSstack[[1]], sizes[strat],
    na.rm = T,
    cells = T
  )[, 1])
  sres <- extract(resids, idx)[, 1] # sampled residuals
...
idx <- as.integer(spatSample(OCSstack[[1]], 1000,
  method = "random",
  na.rm = T, cells = T
)[, 1])
...
dens <- getDensity(
  OCSdata$xcoord * 1000, OCSdata$ycoord * 1000,
  polbuf, 2500
)
plot(dens)
